...
if not iac_files:
    print(f"INFO: No IAC_Database file found in {relative_path.resolve()}, skipping processing.")
...
def transform_assess_data(df):
    ...
    result_df = cost_df.merge(
        usage_df,
        on=['ID'] + id_vars + ['sourccode'],
        how='outer'
    )
    
    # Create ordered categorical for sourccode
    source_order = ['EC', 'ED', 'EF'] + [f'E{i}' for i in range(2, 13)] + [f'W{i}' for i in range(7)]
    result_df['sourccode'] = pd.Categorical(result_df.sourccode, categories=source_order, ordered=True)
...
# Remove records that don't have any values for power sources other than primary
iac_recc_tidy_df = iac_recc_tidy_df.dropna(subset=['sourccode', 'conserved','sourconsv','saved'], how='all')

#------------------------ Integrate IAC ASSESS and RECC datasets ------------------------#

iac_df = pd.merge(iac_assess_tidy_df, iac_recc_tidy_df, 
...
iac_df = iac_df[
    (iac_df['fy'] >= cutoff_year) &
    (iac_df['arc2'].str.startswith("2"))
]

# Filter out rows with sourccode "ED" and "EF" and not starting with "E"
iac_df = iac_df[
    iac_df['sourccode'].str.startswith('E') & 
    ~iac_df['sourccode'].isin(['ED', 'EF'])
]
