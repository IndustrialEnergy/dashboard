...
if not ec_generation_files:
    print(
        f"INFO: No generation file found in {relative_path.resolve()}, skipping processing."
    )
...
if not ec_emissions_files:
    print(
        f"INFO: No emissions file found in {relative_path.resolve()}, skipping processing."
    )
    sys.exit(0)  # Exit cleanly with success code
...
generation_df = generation_df.rename(
    columns={"generation_megawatthours_": "generation_megawatthours"}
)
...
ec_emissions_df = emissions_tidy_df[
    (
...
ec_generation_df = generation_df[
...
ec_emission_factors_df = pd.merge(
    ec_generation_df, ec_emissions_df[["year", "state", "emission_type", "amount"]]
)
ec_emission_factors_df["emission_factor"] = (
    ec_emission_factors_df["amount"]
    / ec_emission_factors_df["generation_megawatthours"]
)
...
ec_emission_factors_df.to_csv(
    "../../data/processed/ec_emission_factors.csv", index=False
)
