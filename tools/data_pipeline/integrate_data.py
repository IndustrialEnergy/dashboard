...
def check_required_files(file_path, required_files):
    ...
    if missing_files:
        ...
        for file in missing_files:
            print(f"  - {file}")
        print(
            "Please ensure all required files are present before running the data integration script."
        )
...
iac_df = pd.read_csv(
    absolute_path / "iac.csv",
    dtype={"sic": str, "naics": str, "payback_imputed": float},
)
...
naics_df = pd.read_csv(
    absolute_path / "sic_to_naics.csv", dtype={"SIC": str, "2002 NAICS": str}
)
# Clean column names
naics_xwalk_df = pd.read_csv(
    absolute_path / "NAICS_SIC_Xwalk.csv",
    dtype={"2022 NAICS Code": str, "Related SIC Code": str},
)
...
integrated_df = pd.merge(
    ...
    fuel_emission_factors_df[
        ["sourccode", "emission_type", "emission_factor", "emission_factor_units"]
    ],
    on="sourccode",
    how="left",
)
...
integrated_df["emissions_avoided"] = (
...
integrated_df["emission_type"] = integrated_df["emission_type_x"].combine_first(
    integrated_df["emission_type_y"]
)
integrated_df["emission_factor_units"] = integrated_df[
    "emission_factor_units_x"
].combine_first(integrated_df["emission_factor_units_y"])
integrated_df["emission_factor"] = integrated_df["emission_factor_x"].combine_first(
    integrated_df["emission_factor_y"]
)
...
ppi_ref_year_df = ppi_ref_year_df[["arc2", "year", "ppi"]].rename(
    columns={"year": "reference_year", "ppi": "reference_ppi"}
)
...
ppi_df = pd.merge(
    ppi_df,
    ppi_ref_year_df[["arc2", "reference_year", "reference_ppi"]],
    on="arc2",
    how="left",
)
...
integrated_df = pd.merge(
    integrated_df,
    ppi_df[["arc2", "reference_year", "reference_ppi", "base_year", "base_ppi"]],
    left_on=["arc2", "base_year"],
    right_on=["arc2", "base_year"],
    how="left",
)
# calculate ppi adjusted implementation cost
integrated_df["impcost_adj"] = integrated_df["impcost"] * (
    integrated_df["reference_ppi"] / integrated_df["base_ppi"]
).round(2)
integrated_df["impcost_adj"] = pd.to_numeric(
    integrated_df["impcost_adj"], errors="coerce"
)
integrated_df["impcost_adj"] = integrated_df["impcost_adj"].round(2)


integrated_df["reference_year"] = pd.to_numeric(
    integrated_df["reference_year"], errors="coerce"
).astype("Int64")

# -------  integrate ARC descriptions ------- #
integrated_df = pd.merge(
    integrated_df, arc_df, left_on="arc2", right_on="specific_code", how="left"
)
...
naics_code_lookup = dict(zip(naics_df["SIC"], naics_df["2002 NAICS"].astype(str)))

naics_2002_desc_lookup = dict(
    zip(naics_df["2002 NAICS"].astype(str), naics_df["2002 NAICS Title"])
)

naics_title_lookup = dict(
    zip(
...
integrated_df["naics_description"] = (
    integrated_df["naics_imputed"].astype(str).map(naics_title_lookup)
)
